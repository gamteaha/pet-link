...
| 2026-06-18 | **Antigravity** (Gemini 3.1 Pro) | **[PC 펫 플레이어 자동 설치 스크립트 추가]**<br>1. `custom-pet-player-extracted/run_pet.bat` 파일에 `electron.exe`가 없을 경우 `npm install`을 자동 실행하는 로직 추가.<br>2. `custom-pet-player-extracted/package.json`에 `electron` 패키지 의존성 및 `electron:start` 스크립트 추가.<br>3. `node_modules` 제외 후 최신 상태로 `dedenne-web/public/releases/custom-pet-player.zip` 재패키징 완료. | 배포판 ZIP 파일 크기 최소화를 위해 `node_modules` 제외 상태로 패키징. 사용자가 ZIP 파일 압축 해제 후 `run_pet.bat` 실행 시, 최초 1회에 한해 필요 모듈(Electron 포함)을 자동 다운로드 및 설치하도록 구현함. |
| 2026-06-18 | **Antigravity** (Gemini 3.1 Pro) | **["나의 펫" 데이터 Supabase DB 마이그레이션]**<br>1. `app/cart/page.tsx`: 펫 다운로드 시 및 장바구니 결제 시 로컬 스토리지뿐만 아니라 Supabase `user_pets` 테이블에 저장하도록 INSERT 구문 추가.<br>2. `app/my-pets/page.tsx`: 로컬 스토리지를 읽기 전 `user_pets`에서 최우선으로 데이터를 로딩하도록 변경.<br>3. `app/my-pets/page.tsx`: 나의 펫 항목에 **🗑️ 삭제** 버튼을 추가하고 Supabase DB 연동 삭제 구현 완료. | 기존 로컬 스토리지 데이터 마이그레이션 여부는 현재 보류. Fallback으로 기존처럼 DB에서 못 불러올 경우 로컬 스토리지를 읽어오도록 구성됨. |
| 2026-06-18 | **Antigravity** (Gemini 3.1 Pro) | **[커스터마이즈 및 3D 모델 버그 수정 & 다운로드 안정화]**<br>1. 커스터마이즈 화면에서 캐릭터 확대 시 이름 입력창이 덮이던 버그 해결 (`overflow-hidden` 적용).<br>2. 강아지에서 고양이로 변경 시 스케일 꼬임(길쭉해짐) 현상을 컴포넌트 강제 리마운트(`key={species}`)로 해결.<br>3. 3D 모델 드래그 시 팽이처럼 회전하는 현상을 `OrbitControls` 차단으로 방지하고 공중에 떴을 때(`startled`) 허공을 걷는 대신 `idle` 상태를 유지하도록 매핑 수정.<br>4. 200MB가 넘던 `custom-pet-player.zip`에서 `node_modules` 등의 빌드 폴더를 완벽히 제거해 1MB 미만으로 재압축 및 `git add -f` 적용으로 Vercel 다운로드 버그(.petlink만 다운되던 현상) 해결 완료.<br>5. `run_pet.bat` 실행 시 Windows CMD에서 LF 줄바꿈 파싱 에러 방지(CRLF) 및 `node_modules`가 이미 존재하더라도 `electron.exe`가 없으면 `npm install`을 강제로 실행하도록 조건식 강화.<br>6. npm v9+에서 발생하는 `electron_mirror` 경고 문구를 제거하기 위해 불필요한 `.npmrc` 삭제. | Vercel 배포 과정에서 `custom-pet-player.zip`이 누락되지 않도록 강제 푸시 처리함. |
| 2026-06-18 | **Antigravity** (Gemini 3.1 Pro) | **[데스크톱 펫 UI 전면 개선]**<br>1. `custom-pet-player-extracted/app/layout.tsx`: 웹샵 전체 레이아웃(`FloatingDecorations`, `SplashScreen`, `pet-bg-pattern` 등)을 제거하고, 투명 배경만 남긴 Electron 전용 최소 레이아웃으로 교체 → 배경 흰색/파란색 오버레이 완전 해결.<br>2. `app/components/CustomPet.tsx`: 이름표 바로 아래에 레벨 + 분홍 호감도 바(`Lv.N ████░░░ ❤️숫자`) 표시 UI 추가.<br>3. `app/components/InventoryWindow.tsx`: 단순 목록을 **🧸용품 / 🍖식품 / 👗옷** 3탭 구조로 전면 재설계. 옷 탭은 빈 슬롯 그리드로 표시. 하단에 **🔴 프로그램 종료** 버튼(2회 클릭 확인 방식) 추가.<br>4. `preload.js` / `main.js`: `quitApp()` IPC 채널 추가 및 종료 핸들러 구현.<br>5. `main.js`: `backgroundColor: '#00000000'` 추가로 Electron 창 로딩 시 흰색 배경 플래시 현상 근본 해결. | **중요**: 데스크톱 플레이어(`custom-pet-player-extracted`)를 수정할 때마다 반드시 `npm run build` 후 `custom-pet-player.zip`을 재패키징하고 `git add -f`로 강제 커밋해야 Vercel에 반영됨. |
| 2026-06-18 | **Antigravity** (Gemini 3.1 Pro) | **[데스크톱 펫 추가 수정 & 웹 버그 수정]**<br>1. `run_pet.bat`: `npm` → `npm.cmd`로 교체(Win32 오류 수정), 오류 발생 시 `pause`로 CMD 유지.<br>2. `CustomPet.tsx`: Y축 이동 완전 제거 → 캐릭터가 항상 바닥 고정, 좌우만 이동. 드래그 후 `setState("idle")` 즉시 복귀.<br>3. `Universal3DViewer.tsx`: `enableRotate` prop 추가 + `trackMouse={false}` 적용으로 마우스 hover 시 모델 회전 완전 제거. Root Motion 트랙 제거로 Y축 뒤틀림 방지.<br>4. `dedenne-web/app/cart/page.tsx`: PC 플레이어 다운로드 섹션 완전 제거. `insert` → `upsert({ onConflict: 'id' })`로 교체하여 동일 펫 중복 추가 방지.<br>5. `dedenne-web/app/customize/page.tsx`: 고양이 탭 클릭 시 기본 털 색 `#E8E8E8`, 강아지 탭 클릭 시 `#D4A574` 자동 적용.<br>6. `main.js`: `process.cwd()` → `__dirname`으로 교체하여 petlink 파일 경로 안정화.<br>7. **`raccoon-pet/`** 폴더가 실제 로컬 테스트 폴더임을 확인 → 모든 수정 파일을 `raccoon-pet/`에도 동기화. | **중요**: 실제 사용자가 로컬 테스트에 사용하는 폴더는 `raccoon-pet/`이며, 배포용 ZIP은 `custom-pet-player-extracted/`에서 생성됨. 두 폴더를 항상 동기화할 것. `raccoon-pet`에는 electron이 devDependency로 없으므로 `npm install electron --save-dev` 후 `node node_modules/electron/install.js`로 바이너리 별도 설치 필요. |
